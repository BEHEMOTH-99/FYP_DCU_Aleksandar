...
public class AnatomicalHandController : MonoBehaviour
{
    ...
    {
        // Default for 9-sensor setup (3 fingers * 3 joints)
        // Joint order: 0=PIP, 1=MCP_V, 2=MCP_H
        for (int f = 0; f < Mathf.Min(fingers.Length, 3); f++)
        {
            int baseIndex = f * 3;
            
            if (baseIndex + 2 < sensorArray.Length)
            {
                // We now receive ABSOLUTE DEGREES from the ESP32
                // We convert them to a 0.0-1.0 scale for the internal smoothing logic
                fingers[f].targetPIP = sensorArray[baseIndex] / PIP_MAX_ANGLE;
                fingers[f].targetMCP_Main = sensorArray[baseIndex + 1] / MCP_MAX_CURL;
                
                // For Splay: 0-30 deg range centered to -1.0 to 1.0 logic
                fingers[f].targetMCP_Secondary = (sensorArray[baseIndex + 2] / MCP_MAX_SPLAY) * 2f - 1f; 
            }
        }
    }
